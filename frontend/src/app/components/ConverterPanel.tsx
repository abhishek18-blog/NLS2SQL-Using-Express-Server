...
export function ConverterPanel({ onConvert, onCancel, currentQuery, currentSql, isLoading, queryUsedForOutput, queryResult, aiResponse, userRole, serverLogs = [] }: ConverterPanelProps) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50 dark:bg-[#09090b] relative overflow-hidden">
      {/* Background decoration */}
      <div className="absolute top-0 inset-x-0 h-64 bg-gradient-to-b from-slate-300/20 dark:from-indigo-500/5 to-transparent pointer-events-none" />
      <div className="absolute -top-32 -left-32 w-96 h-96 bg-slate-200/30 dark:bg-indigo-500/10 blur-[100px] rounded-full pointer-events-none" />
      <div className="absolute -top-32 -right-32 w-96 h-96 bg-slate-100/30 dark:bg-purple-500/10 blur-[100px] rounded-full pointer-events-none" />
      ...
      <div className="flex-1 overflow-y-auto px-6 py-8 md:px-12 md:py-10 z-10 flex flex-col relative max-w-5xl mx-auto w-full">
        ...
        <div className="mb-10 text-center space-y-3 mt-4 relative z-10">
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 dark:from-blue-400 dark:via-indigo-400 dark:to-cyan-400 leading-tight pb-1">
            Natural Language to SQL
          </h1>
          <p className="text-slate-600 dark:text-zinc-400 max-w-2xl mx-auto text-sm md:text-base font-medium">
        ...
        <div className="w-full relative shadow-xl shadow-slate-200/50 dark:shadow-[0_8px_30px_rgb(0,0,0,0.4)] rounded-2xl bg-white dark:bg-zinc-950/60 border border-slate-200 dark:border-zinc-800/80 backdrop-blur-md mb-12 transition-all duration-300 focus-within:border-slate-900 dark:border-indigo-500/50 focus-within:shadow-[0_0_20px_rgba(0,0,0,0.05)] ring-1 ring-white/5">
          ...
          <form onSubmit={handleSubmit} className="p-1 flex flex-col">
            <textarea
              ...
              className="w-full min-h-[140px] bg-transparent rounded-t-2xl px-6 py-5 text-slate-900 dark:text-zinc-100 text-base md:text-lg placeholder:text-zinc-600 focus:outline-none resize-none leading-relaxed transition-colors"
              disabled={isLoading}
            />

            {/* Toolbar inside input */}
            <div className="flex items-center justify-between px-4 py-3 bg-slate-50 dark:bg-zinc-900/80 rounded-b-xl border-t border-slate-200 dark:border-zinc-800/60 mt-auto">
                      ...
                      >
                        ...
                        >
                          <div className="flex flex-col">
                            <span className="text-sm font-medium text-slate-800 dark:text-zinc-200">Sakila DB</span>
                        ...
                        >
                          <div className="flex flex-col">
                            <span className="text-sm font-medium text-slate-800 dark:text-zinc-200">Airport DB</span>
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs text-zinc-500 hidden sm:inline-block">Press <kbd className="font-sans px-1.5 py-0.5 bg-slate-200 dark:bg-zinc-800 rounded text-slate-700 dark:text-zinc-300 text-[10px]">⌘</kbd> + <kbd className="font-sans px-1.5 py-0.5 bg-slate-200 dark:bg-zinc-800 rounded text-slate-700 dark:text-zinc-300 text-[10px]">Enter</kbd></span>
                {isLoading ? (
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  <button
                    type="submit"
                    disabled={!query.trim()}
                    className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-slate-900 dark:from-indigo-500 to-black dark:to-blue-600 hover:from-slate-800 dark:from-indigo-400 hover:to-slate-900 dark:hover:to-blue-500 text-white rounded-xl disabled:from-zinc-800 disabled:to-zinc-800 disabled:text-zinc-500 disabled:cursor-not-allowed transition-all font-semibold text-sm shadow-lg shadow-slate-900/20 dark:shadow-indigo-500/25"
                  >
        ...
        {(currentSql || aiResponse || isLoading) && (
          ...
          >
            {isLoading && !aiResponse && (
              <div className="absolute inset-0 bg-white/80 dark:bg-zinc-950/40 backdrop-blur-sm z-50 flex flex-col items-center justify-center rounded-3xl border border-slate-200 dark:border-zinc-800/60 animate-pulse">
                <div className="w-12 h-12 border-4 border-slate-200 dark:border-indigo-500/20 border-t-slate-900 dark:border-t-indigo-500 rounded-full animate-spin mb-4" />
                <p className="text-slate-600 dark:text-zinc-400 font-medium">AI is analyzing your database...</p>
                <p className="text-xs text-zinc-600 mt-1">This might take a moment depending on the complexity.</p>
              </div>
            )}

            <div className="flex-1 flex flex-col rounded-2xl border border-slate-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-950/80 backdrop-blur-md overflow-hidden shadow-xl shadow-slate-200/50 dark:shadow-2xl ring-1 ring-white/5">
              {/* Context Info */}
              <div className="px-6 py-4 border-b border-slate-200 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/40 flex items-center gap-3 text-sm">
                <div className="p-1.5 bg-slate-200/30 dark:bg-indigo-500/10 rounded-md ring-1 ring-slate-300 dark:ring-indigo-500/20">
                  <Bot className="w-4 h-4 text-slate-900 dark:text-indigo-400" />
                </div>
                <h3 className="font-semibold text-slate-900 dark:text-zinc-100">AI Response</h3>
              </div>

              {/* Chat Bubble Response */}
              <div className="p-6 bg-white dark:bg-zinc-950 relative">
                <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-slate-900 dark:from-indigo-500 to-slate-600 dark:to-cyan-500"></div>
                <p className="text-slate-800 dark:text-zinc-200 text-base leading-relaxed pl-2 font-medium">
              ...
              {queryResult && !(['you need to be an admin', 'admin to access this data'].some(p => (aiResponse || '').toLowerCase().includes(p))) && (
                <div className="border-t border-slate-200 dark:border-zinc-800/60 bg-white dark:bg-zinc-950 flex flex-col flex-1">
                  <div className="flex items-center gap-3 px-6 py-3 border-b border-slate-200 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/30">
                    <TableIcon className="w-4 h-4 text-emerald-400" />
                    <h3 className="text-sm font-semibold text-slate-800 dark:text-zinc-200">Query Results <span className="text-zinc-500 font-normal ml-1">({queryResult.length} rows)</span></h3>
                  </div>
                  <div className="p-4 overflow-x-auto flex-1">
                    <table className="w-full text-left text-sm text-slate-700 dark:text-zinc-300 whitespace-nowrap">
                      <thead className="text-xs uppercase bg-white dark:bg-zinc-900/50 text-slate-600 dark:text-zinc-400">
                        <tr>
                      ...
                      <tbody className="divide-y divide-zinc-800/50">
                        {queryResult.map((row, i) => (
                          <tr key={i} className="hover:bg-white dark:bg-zinc-900/30 transition-colors">
              ...
              <div className="px-6 py-4 bg-white dark:bg-zinc-900/40 border-t border-slate-200 dark:border-zinc-800/40 flex flex-col gap-2">
