...
export function Sidebar({ history, onSelectHistory, onClearHistory, selectedId }: SidebarProps) {
  ...
  return (
    <div className="w-80 bg-white dark:bg-zinc-950 border-r border-slate-200 dark:border-zinc-800/80 flex flex-col h-full z-20 shadow-2xl">
      {/* Header */}
      <div className="p-6 pb-4 border-b border-slate-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-950/90 backdrop-blur sticky top-0">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-slate-900 dark:text-zinc-100 font-semibold flex items-center gap-2">
            <History className="w-5 h-5 text-slate-900 dark:text-indigo-400" />
            Query History
          </h2>
          <span className="bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-slate-500 dark:text-zinc-400 text-xs px-2.5 py-1 rounded-full font-medium shadow-inner">
        ...
        <div className="relative group">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 dark:text-zinc-500 w-4 h-4 transition-colors group-focus-within:text-slate-900 dark:text-indigo-400" />
          <input
            type="text"
            placeholder="Search saved queries..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-slate-100 dark:bg-zinc-900/50 border border-slate-200 dark:border-zinc-800/80 rounded-xl pl-10 pr-3 py-2.5 text-sm text-slate-900 dark:text-zinc-200 placeholder-slate-400 dark:placeholder-zinc-500 focus:outline-none focus:border-slate-900 dark:border-indigo-500/50 focus:ring-1 focus:ring-slate-900 dark:ring-indigo-500/50 transition-all shadow-sm"
          />
        </div>
      </div>

      {/* History List */}
      <div className="flex-1 overflow-y-auto px-4 py-3 bg-white dark:bg-zinc-950 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent">
        {filteredHistory.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-slate-500 dark:text-zinc-500 space-y-3 opacity-80">
            <Clock className="w-8 h-8 text-slate-400 dark:text-zinc-700" />
            <p className="text-sm">
              ...
              >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className={`text-sm font-medium mb-1.5 leading-snug line-clamp-2 ${selectedId === item.id ? 'text-slate-900 dark:text-indigo-100' : 'text-slate-700 dark:text-zinc-300'}`}>
                      {item.query}
                    </h4>
      ...
      {history.length > 0 && (
        <div className="p-4 border-t border-slate-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-950/90 backdrop-blur">
          <button
